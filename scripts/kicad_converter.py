...
"""
    ...
    def generate_schematic_file(self, output_file: Path, circuit: Dict):
        ...
        if multi_unit_symbols:
            print(f"  → Found {len(multi_unit_symbols)} multi-unit symbols, generating power units...")

        # Generate power unit instance for each multi-unit symbol
        power_unit_y_offset = -50.08  # Place power units below schematic
        power_unit_coords = {}  # Track power unit positions for wire generation
        ...
        for idx, (symbol, info) in enumerate(multi_unit_symbols.items()):
            ...
            # Position power units in a row at bottom of schematic
            x = 50.8 + (idx * 25.4)
            y = power_unit_y_offset
            power_unit_coords[ref] = (x, y)  # Store for wire generation
            comp_uuid = str(uuid.uuid4())

            lines.append(f'  (symbol')
            lines.append(f'    (lib_id "{symbol}")')
            lines.append(f'    (at {self._round_coordinate(x)} {self._round_coordinate(y)} 0)')
            lines.append(f'    (unit {power_unit_num})')  # Power unit!
            lines.append(f'    (exclude_from_sim no)')
            lines.append(f'    (in_bom yes)')
            lines.append(f'    (on_board yes)')
            lines.append(f'    (dnp no)')
            ...
            for pin_num in unit_info['power_pins']:
                # Guess which net based on typical power pin numbers
                # Pin 4 or 11 = V-, Pin 8 or 14 = V+
                net_assigned = None
                if pin_num in ['4', '11']:
                    if power_nets['gnd']:
                        pin_net_mapping[f"{ref}.{pin_num}"] = power_nets['gnd']
                        net_assigned = power_nets['gnd']
                    elif power_nets['vneg']:
                        pin_net_mapping[f"{ref}.{pin_num}"] = power_nets['vneg']
                        net_assigned = power_nets['vneg']
                elif pin_num in ['8', '14']:
                    if power_nets['vcc']:
                        pin_net_mapping[f"{ref}.{pin_num}"] = power_nets['vcc']
                        net_assigned = power_nets['vcc']
                    elif power_nets['vpos']:
                        pin_net_mapping[f"{ref}.{pin_num}"] = power_nets['vpos']
        ...
        for net_name in nets:
            ...
            # TC #66: Place ONE global label per net at the first wire stub endpoint
            if wire_endpoints and net_name not in labeled_nets:
                first_stub_x, first_stub_y, first_pin_id = wire_endpoints[0]

                # Determine label justify based on position (simplified)
                justify = 'left'
                ...
                lines.append(f'    (at {self._format_coordinate(first_stub_x, snap_to_grid=False)} {self._format_coordinate(first_stub_y, snap_to_grid=False)} 0)')
            ...
            # TC #66: For multi-pin nets, connect wire stub endpoints together
            # Use chain topology: stub1 → stub2 → stub3 → ... → stubN
            if len(wire_endpoints) > 1:
                for i in range(len(wire_endpoints) - 1):
                    x1, y1, _ = wire_endpoints[i]
                    x2, y2, _ = wire_endpoints[i + 1]

                    # Generate connecting wire
                    conn_wire_uuid = str(uuid.uuid4())
                    lines.append(f'  (wire')
                    lines.append(f'    (pts')
                    lines.append(f'      (xy {self._format_coordinate(x1, snap_to_grid=False)} {self._format_coordinate(y1, snap_to_grid=False)})')
                    lines.append(f'      (xy {self._format_coordinate(x2, snap_to_grid=False)} {self._format_coordinate(y2, snap_to_grid=False)})')
                    lines.append(f'    )')
                    lines.append(f'    (stroke')
                    lines.append(f'      (width 0)')
                    lines.append(f'      (type default)')
                    lines.append(f'    )')
                    lines.append(f'    (uuid "{conn_wire_uuid}")')
                    lines.append(f'  )')
                    wire_count += 1
